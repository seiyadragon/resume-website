...
export const Project = ({details, images, url}: ProjectProps) => {
    ...
    return (
        <section className="lg:w-1/4 w-1/2 lg:h-64">
            <button 
                className="mt-4 bg-blue-400 w-full text-center py-2 transition hover:scale-105 text-slate-900 text-xl shadow-xl"
                onClick={() => setMenuOpen(!menuOpen)}
            >
                {details.name}
            </button>
            {menuOpen &&
                <section className="w-full text-slate-900 bg-blue-400 px-4 py-4 shadow-xl overflow-y-scroll h-64">
                    ...
                    <ul className="py-4">
                        {images.map((image) => {
                            return (
                                <li key={image} className="py-2">
                                    <Image className="shadow-lg" src={image} alt="Cant Load Image." width={1920} height={1080} />
                                </li>
                            )
                        })}
                    </ul>
...
const ProjectArea = () => {
    return (
        <section className="py-8 lg:w-1/2 w-full">
            <span 
                className="flex flex-col items-center text-4xl text-slate-900 bg-blue-400 py-8 shadow-xl w-full"
            >
                Projects
            </span>
