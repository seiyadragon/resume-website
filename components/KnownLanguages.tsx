...
export const LanguageSection = ({language, description}: LanguageSectionProps) => {
    ...
    return (
        <section className="lg:w-1/4 w-1/2 lg:h-64">
            <button 
                className="mt-4 bg-blue-400 w-full text-center py-2 transition hover:scale-105 shadow-lg text-xl"
                onClick={() => setMenuActive(!isMenuActive)}
            >
                {language}
            </button>
            {isMenuActive &&
                <p className="bg-blue-400 w-full py-4 px-4 text-slate-900 text-lg shadow-lg h-64 overflow-y-scroll">
                    {description}
                </p>
            }
        </section>
...
const KnownLanguages = () => {
    return (
        <section className="flex flex-col items-center text-slate-900 py-8 lg:w-1/2 w-full">
